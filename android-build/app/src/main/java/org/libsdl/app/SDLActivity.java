...
public class SDLActivity
    extends Activity implements View.OnSystemUiVisibilityChangeListener {
  ...
  /*
      ...
      //
      // SDLActivity.debugSource(device.getSources(), "device[" +
     device.getName() + "]");
      ...
      public static void debugSource(int sources, String prefix) {
          ...
          if ((s & InputDevice.SOURCE_CLASS_JOYSTICK) != 0)   cls += "
     JOYSTICK"; if ((s & InputDevice.SOURCE_CLASS_POINTER) != 0)    cls += "
     POINTER"; if ((s & InputDevice.SOURCE_CLASS_POSITION) != 0)   cls += "
     POSITION"; if ((s & InputDevice.SOURCE_CLASS_TRACKBALL) != 0)  cls += "
     TRACKBALL";
          ...
          Log.v(TAG, prefix + "int=" + s_copy + " CLASS={" + cls + " }
     source(s):" + src);
      }
  */
  ...
  public static final boolean mHasMultiWindow =
      (Build.VERSION.SDK_INT >= 24 /* Android 7.0 (N) */);
   ...
   * getMainFunction() methods to invoke native code from the specified shared
   * library. It can be overridden by derived classes.
   */
  protected void main() {
  ...
  /**
   * This method returns the name of the shared object with the application
   * entry point It can be overridden by derived classes.
   */
   ...
   */
  protected String[] getLibraries() { return new String[] {"GrnGame"}; }
  ...
  public static void initialize() {
    ...
    // initialize everything here Otherwise, when exiting the app and returning
    // to it, these variables *keep* their pre exit values
    mSingleton = null;
    mSurface = null;
    mTextEdit = null;
    mLayout = null;
  ...
  protected void onCreate(Bundle savedInstanceState) {
    ...
    if (!mBrokenLibraries) {
      ...
      if (!version.equals(expected_version)) {
        mBrokenLibraries = true;
        errorMsgBrokenLib = "SDL C/Java version mismatch (expected " +
    ...
    if (mBrokenLibraries) {
      ...
      dlgAlert.setMessage("An error occurred while trying to start the "
    ...
    {
      int run_count =
          SDLActivity.nativeCheckSDLThreadCounter(); /* get and increment a
                                                        native counter */
      if (run_count != 0) {
        boolean allow_recreate = SDLActivity.nativeAllowRecreateActivity();
        if (allow_recreate) {
  ...
  public void onWindowFocusChanged(boolean hasFocus) {
    ...
    // If we are gaining focus, we can always try to restore our USB devices. If
    // we are losing focus, only try to relinquish them if we don't have
    // background events allowed (for multi-window Android setups).
    if (hasFocus || !SDLActivity.nativeGetHintBoolean(
                        "SDL_JOYSTICK_ALLOW_BACKGROUND_EVENTS", false)) {
      if (mHIDDeviceManager != null) {
  ...
  protected void onDestroy() {
    ...
    if (SDLActivity.mSDLThread != null) {
      ...
      try {
        ...
        // and so the previous QUIT event will be discarded by SDL_Init() and
        // app is running, not exiting.
  ...
  public void onBackPressed() {
    ...
    //
    // If we do, the normal hardware back button will no longer work and people
    // have to use home, but the mouse right click will work.
    //
    boolean trapBack =
        SDLActivity.nativeGetHintBoolean("SDL_ANDROID_TRAP_BACK_BUTTON", false);
    if (trapBack) {
  ...
  public static void manualBackButton() { mSingleton.pressBackButton(); }
  ...
  public static boolean dispatchingKeyEvent() { return mDispatchingKeyEvent; }
  ...
  public static void handleNativeState() {
    ...
    if (mNextNativeState == NativeState.RESUMED) {
      if (mSurface.mIsSurfaceReady && (mHasFocus || mHasMultiWindow) &&
          mIsResumedCalled) {
        if (mSDLThread == null) {
    ...
    public void handleMessage(Message msg) {
      ...
      case COMMAND_TEXTEDIT_HIDE:
        if (mTextEdit != null) {
          ...
          // landscape. Setting the View's sizes to 0 is similar to GONE but
          // without the flicker. The sizes will be set to useful values when
          // the keyboard is shown again.
  ...
  protected boolean sendCommand(int command, Object data) {
    ...
    if (command == COMMAND_CHANGE_WINDOW_STYLE) {
      ...
      if (data instanceof Integer) {
        ...
        boolean bFullscreenLayout =
            ((realMetrics.widthPixels == mSurface.getWidth()) &&
             (realMetrics.heightPixels == mSurface.getHeight()));

        if ((Integer)data == 1) {
          // If we aren't laid out fullscreen or actively in fullscreen mode
          // already, we're going to change size and should wait for
          // surfaceChanged() before we return, so the size is right back in
          // native code.  If we're already laid out fullscreen, though, we're
          // not going to change size even if we change decor modes, so we
          // shouldn't wait for surfaceChanged() -- which may not even happen --
          // and should return immediately.
          bShouldWait = !bFullscreenLayout;
        } else {
          // If we're laid out fullscreen (even if the status bar and nav bar
          // are present), or are actively in fullscreen, we're going to change
          // size and should wait for surfaceChanged before we return, so the
          // size is right back in native code.
          bShouldWait = bFullscreenLayout;
        }
      }
  ...
  nativeSetScreenResolution(int surfaceWidth, int surfaceHeight,
                            int deviceWidth, int deviceHeight, float density,
                            float rate);
  ...
  public static native void onNativeTouch(int touchDevId, int pointerFingerId,
                                          ...
                                          float p);
  public static native void onNativePen(int penId, int device_type, int button,
  ...
  public static native void onNativeInsetsChanged(int left, int right, int top,
                                                  int bottom);
  ...
  public static native void onNativeFileDialog(int requestCode,
                                               String[] filelist, int filter);
  ...
  public void setOrientationBis(int w, int h, boolean resizable, String hint) {
    ...
    if (!is_portrait_allowed && !is_landscape_allowed) {
      if (resizable) {
        /* All orientations are allowed, respecting user orientation lock
         * setting */
        req = ActivityInfo.SCREEN_ORIENTATION_FULL_USER;
      } else {
        /* Fixed window and nothing specified. Get orientation from w/h of
         * created window */
      ...
      if (resizable) {
        if (is_portrait_allowed && is_landscape_allowed) {
          ...
          req = (is_landscape_allowed ? orientation_landscape
    ...
    Log.v(TAG, "setOrientation() requestedOrientation=" + req + " width=" + w +
                   " height=" + h + " resizable=" + resizable +
                   " hint=" + hint);
  ...
  public static boolean supportsRelativeMouse() {
    // DeX mode in Samsung Experience 9.0 and earlier doesn't support relative
    // mice properly under Android 7 APIs, and simply returns no data under
    // Android 8 APIs.
    //
    // This is fixed in Samsung Experience 9.5, which corresponds to
    // Android 8.1.0, and thus SDK version 27.  If we are in DeX mode and not
    // API 27 or higher, as a result, we should stick to relative mode.
    //
   ...
   * This method is called by SDL using JNI.
   */
  public static Activity getContext() { return SDL.getContext(); }
  ...
  public static boolean isAndroidTV() {
    ...
    if (Build.MANUFACTURER.equals("Amlogic") &&
        (Build.MODEL.startsWith("TV") || Build.MODEL.equals("X96-W") ||
  ...
  public static boolean isVRHeadset() {
    if (Build.MANUFACTURER.equals("Oculus") &&
  ...
  public static double getDiagonal() {
    ...
    return Math.sqrt((dWidthInches * dWidthInches) +
                     (dHeightInches * dHeightInches));
  }
  ...
  public static boolean isDeXMode() {
    ...
    try {
      ...
      return configClass.getField("SEM_DESKTOP_MODE_ENABLED")
                 .getInt(configClass) ==
   ...
   * This method is called by SDL using JNI.
   */
  public static boolean showTextInput(int input_type, int x, int y, int w,
                                      int h) {
  ...
  public static boolean isTextInputEvent(KeyEvent event) {
    ...
    return event.isPrintingKey() ||
        event.getKeyCode() == KeyEvent.KEYCODE_SPACE;
  }

  public static boolean handleKeyEvent(View v, int keyCode, KeyEvent event,
                                       InputConnection ic) {
    ...
    //            Log.v("SDL", "key down: " + keyCode + ", deviceId = " +
    //            deviceId + ", source = " + source);
    //        } else if (event.getAction() == KeyEvent.ACTION_UP) {
    //            Log.v("SDL", "key up: " + keyCode + ", deviceId = " + deviceId
    //            + ", source = " + source);
    //        }

    // Dispatch the different events depending on where they come from
    // Some SOURCE_JOYSTICK, SOURCE_DPAD or SOURCE_GAMEPAD are also
    // SOURCE_KEYBOARD So, we try to process them as JOYSTICK/DPAD/GAMEPAD
    // events first, if that fails we try them as KEYBOARD
    //
    ...
    if ((source & InputDevice.SOURCE_MOUSE) == InputDevice.SOURCE_MOUSE) {
      if (SDLActivity.isVRHeadset()) {
        // The Oculus Quest controller back button comes in as source mouse, so
        // accept that
      } else {
        // on some devices key events are sent for mouse BUTTON_BACK/FORWARD
        // presses they are ignored here because sending them as mouse input to
        // SDL is messy
  ...
  public static void initTouch() {
    ...
    for (int id : ids) {
      ...
      /* Allow SOURCE_TOUCHSCREEN and also Virtual InputDevices because they can
       * send TOUCHSCREEN events */
      if (device != null &&
   ...
   */
  public int messageboxShowMessageBox(final int flags, final String title,
    ...
    if ((buttonFlags.length != buttonIds.length) &&
  ...
  protected void messageboxCreateAndShow(Bundle args) {
    ...
    dialog.setOnDismissListener(new DialogInterface.OnDismissListener() {
      ...
      public void onDismiss(DialogInterface unused) {
        synchronized (messageboxSelection) { messageboxSelection.notify(); }
      }
    });
    ...
    for (int i = 0; i < buttonTexts.length; ++i) {
      ...
      if (buttonBackgroundColor != Color.TRANSPARENT) {
        ...
        if (drawable == null) {
          ...
          // setting the color this way keeps the style (gradient, padding,
          // etc.)
          drawable.setColorFilter(buttonBackgroundColor,
    ...
    if (Build.VERSION.SDK_INT >= 24 /* Android 7.0 (N) */) {
      try {
        mSurface.setPointerIcon(
            PointerIcon.getSystemIcon(getContext(), cursor_type));
      } catch (Exception e) {
        return false;
      }
    }
  ...
  public static boolean showToast(String message, int duration, int gravity,
    ...
    try {
      class OneShotTask implements Runnable {
        ...
        OneShotTask(String message, int duration, int gravity, int xOffset,
                    int yOffset) {
          mMessage = message;
          mDuration = duration;
          mGravity = gravity;
  ...
  public static int openFileDescriptor(String uri, String mode)
    ...
    try {
      ParcelFileDescriptor pfd =
          mSingleton.getContentResolver().openFileDescriptor(Uri.parse(uri),
                                                             mode);
  ...
  public static String formatLocale(Locale locale) {
    ...
    if (locale.getLanguage() == "in") {
      // Indonesian is "id" according to ISO 639.2, but on Android is "in"
      // because of Java backwards compatibility
      lang = "id";
