...
---@param rotation integer Rotation in degrees
---@return Particle
function image.Particle(x, y, r, g, b, a, rotation) end
...
---@param numberOfJumpsPossible? number (Optional) number of jumps possible
---@param canJumpOnWall? boolean (Optional) the entity can jump on wall ?
...
function game.EntityPlatformer(x, y, w, h, jumpPower, gravity, speedMaxX, speedMaxFall, wallCorrection, initialSpeed,
                               acceleration, numberOfJumpsPossible, canJumpOnWall)
end

--- Creates a container for Platformer Entities.
---@return EntitiesPlatformer
function game.EntitiesPlatformer() end
...
function game.hitboxPlatformer(entities, blocks, ignore_mask, delta) end

--- Enables or disables the visual debugging of hitboxes.
--- When active, it draws outlines around the player, entities, and collision blocks.
---
--- @param display boolean If true, hitboxes are rendered; if false, they are hidden
function game.setDebugHitbox(display) end
...
---@field numberOfJumps number
---@field numberOfJumpsPossible number
...
--- Returns the size of the container (number of particles).
---@return integer
...
--- Returns an iterator for the particles to be used in a for...in loop.
