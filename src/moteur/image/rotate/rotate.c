...
/* Cree une nouvelle texture en appliquant une rotation pixel par pixel */
static SDL_Texture *creer_texture_tournee_pixelart(SDL_Texture *src, Uint16 w, Uint16 h,
    ...
    /* Calcul de la nouvelle taille englobante basee sur la rotation */
    int new_w = (int)ceil(fabs(w * cos_a) + fabs(h * sin_a));
    int new_h = (int)ceil(fabs(w * sin_a) + fabs(h * cos_a));

    /* Creation texture temporaire pour lire les pixels source */
    ...
    Uint32 *pixels_dst = (Uint32 *)malloc_gestion_echec_compteur(new_w * new_h * sizeof(Uint32));
    ...
    /* Rotation manuelle: chaque pixel destination est calcule par rotation inverse */
    for (int y_dst = 0; y_dst < new_h; y_dst++) {
        for (int x_dst = 0; x_dst < new_w; x_dst++) {
            /* Coordonnees relatives au centre de rotation */
            ...
            /* Application du miroir horizontal si necessaire */
            ...
            /* Copie du pixel si les coordonnees sont valides */
            if (ix >= 0 && ix < w && iy >= 0 && iy < h)
                pixels_dst[y_dst * new_w + x_dst] = pixels_src[iy * w + ix];
        }
    }

    /* Creation de la texture SDL finale avec interpolation Nearest Neighbor */
    ...
    free_gestion_echec_compteur(pixels_src);
    free_gestion_echec_compteur(pixels_dst);
...
/* Optimise la rotation pour les angles droits (90, 180, 270 degres) */
SDL_Texture *creer_texture_tournee_rapide(SDL_Texture *src, Uint16 w, Uint16 h, Uint16 angle,
    ...
    /* Creation de la texture finale avec les pixels permutes */
    SDL_Texture *texture_tournee =
        SDL_CreateTexture(rendu, SDL_PIXELFORMAT_RGBA8888, SDL_TEXTUREACCESS_STATIC, new_w, new_h);
    if (texture_tournee) {
        SDL_SetTextureBlendMode(texture_tournee, SDL_BLENDMODE_BLEND);
