package("libsdl3")
    ...
    if is_plat("wasm") then
        add_cxflags("-sUSE_SDL=0", "-pthread", "-matomics", "-mbulk-memory")
    end
    ...
    on_install(function (package)
        ...
        if package:is_plat("wasm") then
            -- ensure SDL3's own CMake threading detection picks the pthread backend
            -- instead of silently falling back to SDL_THREAD_GENERIC
        ...
        elseif package:is_plat("wasm") then
            ...
            -- adding flags by security
            cflags = {"-sUSE_SDL=0", "-pthread", "-matomics", "-mbulk-memory"}
            ldflags = {"-pthread"}
        end
        ...
        import("package.tools.cmake").install(package, configs, {cflags = cflags, ldflags = ldflags})
    end)
