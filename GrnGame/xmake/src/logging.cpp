...
NB_MODULE(LibGrnGame, m) {
    // Désactive les warnings de leak (si possible)
    ...
    m.def("changer_log",&changer_niveau_log);
    //init
    ...
    m.def("boucle_principale", [](Gestionnaire* jeu) {
        boucle_principale(jeu);
        // corection erreur GIL
        gUpdateCallback = nb::object();
    });
