...

{
    ...
    jeu->shader = (Shader*)malloc(sizeof(Shader));
    ...
    if (!jeu->fond || !jeu->entrees || !jeu->image || !jeu->textures || !jeu->sons ||!jeu->fond)
    {
    ...
    jeu->shader->g = NULL;
    jeu->shader->b = NULL;
    jeu->shader->posx = NULL;
    jeu->shader->posy = NULL;
    jeu->shader->taille = 0;
    ...
    jeu->image->tab = (Object_image*)malloc(sizeof(Object_image) * jeu->image->capacite_images);
    if (!jeu->image->tab)
    {
    ...
    memset(jeu->image->tab, 0, sizeof(Object_image) * jeu->image->capacite_images);
