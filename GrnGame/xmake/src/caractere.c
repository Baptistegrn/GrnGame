...
#include <string.h>
 ...
 float ajouter_char_dans_tableau(Gestionnaire *jeu, const char *lien_image, 
                                        char lettre, float posx, float posy, 
    ...
    if (SDL_QueryTexture(texture, NULL, NULL, &texW, &texH) != 0) {
        fprintf(stderr, "Erreur: Impossible d'obtenir les dimensions de la texture '%s': %s\n", 
                lien_image_lettre, SDL_GetError());
        return 0.0f;
    }
    ...
    ajouter_image_au_tableau(jeu, lien_image_lettre, posx, posy, 
                            largeur_finale, hauteur_finale, sens, rotation);

    return largeur_finale;
}

void ajouter_mot_dans_tableau(Gestionnaire *jeu, const char *chemin, 
                                       const char *mot, float posx, float posy, 
