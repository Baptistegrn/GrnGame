...
int collect_wavs(const char* dir, char*** out_list, int* out_count)
{
    ...
    {
        log_fmt(NiveauLogErreur, "Erreur: Paramètres NULL dans la collecte de fichier son .wav dans le dossier");
        return -1;
    }
    ...
    do
    {
        ...
        else if (ends_with_wav(fd.cFileName))
        {
            ...
            if (!tmp)
            {
                log_fmt(NiveauLogErreur, "Erreur: Échec de réallocation mémoire lors de la collecte des WAV");
                FindClose(hFind);
                return -1;
            }
