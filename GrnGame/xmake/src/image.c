...
void ajouter_image_au_jeu(Gestionnaire* gestionnaire, image nouvelle)
{
    ...
    if (jeu->nb_images >= jeu->capacite_images)
    {
        ...
        if (!nouveau_tab)
        {
            log_fmt(NiveauLogErreur, "Erreur: Échec de réallocation mémoire pour les images (capacité: %d -> %d)",
                              ...
                              int rotation,int transparence)
{
    if (!gestionnaire)
    {
        log_fmt(NiveauLogErreur, "Erreur: Gestionnaire NULL dans ajouter_image_au_tableau()");
        return;
    }

    if (!gestionnaire->textures)
    {
        log_fmt(NiveauLogErreur, "Erreur: Gestionnaire de textures non initialisé");
        return;
    }
    ...
    if (!tex)
    {
        log_fmt(NiveauLogErreur, "Erreur: Texture introuvable '%s'", id);
        return;
    }
                              ...
                              int rotation,int transparence,int r,int g, int b)
{
    if (!gestionnaire)
    {
        log_fmt(NiveauLogErreur, "Erreur: Gestionnaire NULL dans ajouter_forme_au_tableau()");
        return;
    }
    image img;
    memset(&img, 0, sizeof(image));
                                    ...
                                    int* sens, int* rotation,int* transparence, int taille)
{
    if (!gestionnaire)
    {
        log_fmt(NiveauLogErreur, "Erreur: Gestionnaire NULL dans ajouter_image_au_tableau_batch()");
        return;
    }

    if (taille <= 0)
    {
        log_fmt(NiveauLogErreur, "Erreur: Taille de batch invalide (%d)", taille);
        return;
    }
...
void afficher_images(Gestionnaire* gestionnaire)
{
    ...
    {
        log_fmt(NiveauLogErreur, "Erreur: Composants manquants pour afficher les images");
        return;
    }
    ...
    for (int i = 0; i < jeu->nb_images; i++)
    {
        ...
        if (img->forme)
        {
            if (img->rotation == 0 && img->sens == 0)
            {
                ...
                {
                    log_fmt(NiveauLogErreur, "Erreur SDL_RenderFillRect : %s", SDL_GetError());
                }
            }
            else
            {
                ...
                {
                    log_fmt(NiveauLogErreur, "Erreur SDL_RenderGeometry : %s", SDL_GetError());
                }
            }
        }
        else
        {
            ...
            if (img->rotation == 0.0f && img->sens == 0)
            {
                ...
                {
                    log_fmt(NiveauLogErreur, "Erreur rendu texture %d: %s", i, SDL_GetError());
                }
            }
            else
            {
...
void dessiner_bandes_noires(SDL_Renderer* rendu, double decalage_x, double decalage_y, int largeur, int hauteur)
{
    if (!rendu)
    {
        log_fmt(NiveauLogErreur, "Erreur: Renderer NULL dans dessiner_bandes_noires()");
        return;
    }
...
void actualiser(Gestionnaire* jeu, bool bande_noir)
{
    if (!jeu || !jeu->rendu)
    {
        log_fmt(NiveauLogErreur, "Erreur: Gestionnaire ou renderer NULL dans actualiser()");
        return;
    }
