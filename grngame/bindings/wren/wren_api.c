...
static void WrenSetCallHandle()
{
    for (int i = 0; i <= 16; i++)
    {
        g_app.wren->call[i] = wrenMakeCallHandle(g_app.wren->vm, CALL_SIGNATURES[i]);
    }
}
...
bool WrenLoadMainHandles(const char *main_module_name)
{
    ...
    g_app.wren->arity = wrenMakeCallHandle(g_app.wren->vm, "arity");
    return true;
}

static bool CallMainNoArgHandle(WrenHandle *handle, const char *method_name)
{
    if (!g_app.wren->main_class || !handle)
        return false;

    wrenEnsureSlots(g_app.wren->vm, 1);
    wrenSetSlotHandle(g_app.wren->vm, 0, g_app.wren->main_class);
    return CheckWrenCallResult(wrenCall(g_app.wren->vm, handle), method_name);
}
...
{
    return CallMainNoArgHandle(g_app.wren->on_start, "on_start");
}

bool WrenCallOnUpdate(float32 delta)
{
    return CallMainDeltaHandle(g_app.wren->on_update, delta, "on_update");
}

bool WrenCallOnFixedUpdate(float32 delta)
{
    return CallMainDeltaHandle(g_app.wren->on_fixed_update, delta, "on_fixed_update");
}

bool WrenCallOnRender()
{
    return CallMainNoArgHandle(g_app.wren->on_render, "on_render");
}

bool WrenCallOnDestroy()
{
    return CallMainNoArgHandle(g_app.wren->on_destroy, "on_destroy");
}
...
void WrenFree()
{
    ...
    SafeReleaseHandle(g_app.wren->vm, &g_app.wren->arity);

    for (int i = 0; i < 4; i++)
    {
        if (g_app.wren->registry.callbacks[i].is_registered)
        {
            SafeReleaseHandle(g_app.wren->vm, &g_app.wren->registry.callbacks[i].handle);
            g_app.wren->registry.callbacks[i].is_registered = false;
        }
    }
...
bool CallWrenCallback(int16 index, void *data, uint8 arg_count)
{
    ...
    WrenCallback *cb = &g_app.wren->registry.callbacks[index];
    ...
    WrenInterpretResult result = wrenCall(vm, g_app.wren->call[arg_count]);
