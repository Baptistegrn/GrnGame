...
void Event_callback(WrenVM* vm)
{
    ...
    if (g_app.wren->registry.callbacks[index].is_registered) {
        wrenReleaseHandle(vm, g_app.wren->registry.callbacks[index].handle);
    }

    wrenEnsureSlots(vm, 1);
    wrenSetSlotHandle(vm, 0, fn);
    wrenCall(vm, g_app.wren->arity);
    uint8 arity = (uint8)wrenGetSlotDouble(vm, 0);

    g_app.wren->registry.callbacks[index].handle = fn;
    g_app.wren->registry.callbacks[index].is_registered = true;
    g_app.wren->registry.callbacks[index].arity = arity;
}
