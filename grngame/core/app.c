...
static COLD void EnsureInitSucceeded(InitResult res)
{
    if (UNLIKELY(res == INIT_SDL_FAILED)) {
        MessageBox("Failed to initialize SDL.");
        exit(1);
    }

    if (UNLIKELY(res == INIT_LOG_FAILED)) {
        MessageBox("Failed to initialize the logging system.");
        exit(2);
    }
    
    if (UNLIKELY(res == INIT_CONFIG_FAILED)) {
        MessageBox("Failed to load or parse the configuration file.");
        exit(3);
    }
    
    if (UNLIKELY(res == INIT_OPEN_GAME_DATA_FAILED)) {
        MessageBox("Failed to open game data.");
        exit(4);
    }
}
